...
export const injectOpenGraphTags = (metadata: OpenGraphMetadata) => {
  const baseMetaTags = [
    ...
    { property: 'og:site_name', content: 'TechNexos' },
    ...
    { name: 'keywords', content: `${metadata.title}, blog, tecnologia, consultoria` },
  ]
  ...
  // Update title
  document.title = `${metadata.title} | TechNexos Blog`
...
export const fetchBlogMetadata = async (slug: string): Promise<OpenGraphMetadata | null> => {
  try {

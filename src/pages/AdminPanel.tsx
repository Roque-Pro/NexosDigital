...
const AdminPanel = () => {
     ...
     const [activeTab, setActiveTab] = useState<"employees" | "inventory" | "patrimonio" | "financial">("employees");
     const [activeFinancialTab, setActiveFinancialTab] = useState<"receitas" | "despesas" | "patrimonio" | "estoque">("despesas");
    ...
    useEffect(() => {
      const loadData = async () => {
        setLoading(true);
        await Promise.all([fetchEmployees(), fetchProducts(), fetchAssets(), fetchExpenses()]);
        setLoading(false);
      };
      loadData();
    }, []);
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-muted-foreground mb-1">Valor Total em Estoque</p>
                                    <p className="text-2xl font-bold text-primary">R$ {totalInventoryValue.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p>
                                </div>
                                <div className="w-12 h-12 rounded-lg bg-primary/15 flex items-center justify-center">
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-sm text-muted-foreground mb-1">Top Categoria</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-success">R$ {(
                    (employees.reduce((sum, e) => sum + (e.salary || 0), 0) * -1) +
                    totalInventoryValue
                  ).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p>
                  <p className="text-xs text-muted-foreground mt-1">Serviços + Planos</p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-primary">R$ {(
                    ((employees.reduce((sum, e) => sum + (e.salary || 0), 0) * -1) + totalInventoryValue) -
